...
function cerber_settings_config( $args = array() ) {
    ...
    // Declarations of the plugin settings and UI fields split into admin page sections
	...
	if ( ! $sections ) {
		...
		$sections = array(
			...
			'citadel'   => array(
				...
				'section_desc'   => __( 'In the Citadel mode nobody is able to log in except IPs from the White IP Access List. Active user sessions will not be affected.', 'wp-cerber' ),
				'fields' => array(
					'citadel_on'  => array(
			...
			'hwp'  => array(
				...
				'section_desc'   => __( 'These restrictions do not apply to IP addresses in the White IP Access List', 'wp-cerber' ),
				'fields' => array(
					'stopenum'            => array(
...
/**
 * Top-level processor of settings-related things.
 *
 * @return void
 */
function crb_settings_processor() {

    // We handle only these three locations
    ...
    // Configure WordPress to render standard WP settings forms for the current admin page
...
function cerber_wp_settings_setup( $screen_id, $sections = array() ) {
	...
	foreach ( $sections as $section_id => $section_config ) {
        ...
        if ( $deck_items ) {
            $section_deck_html = '<div class="crb-setting-section-deck">' . implode( "\n", $deck_items ) . '</div>';
        }

		add_settings_section(
			$section_id,
			crb_array_get( $section_config, 'name', '' ),
			function () use ( $section_deck_html ) {
				echo $section_deck_html;
			},
			$option
		);
...
function cerber_process_settings_form() {
	...
	if ( ! nexus_is_valid_request() ) {
        ...
        // Standard local context: we check permissions and nonces

		if ( ! cerber_user_can_manage() ) {
			return;
		}

        // Abort processing if the setting form nonce is not valid or missing.
		// See wp_nonce_field() in the settings_fields() function
		check_admin_referer( $_POST['option_page'] . '-options' );
	}

    // All checks are passed, now we are updating settings using submitted form fields

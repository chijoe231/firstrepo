...
*/
function crb_acl_build_table_view( string $tag, int $slice_id = 0 ): string {
    global $wpdb;
...
function cerber_show_admin_messages() {
	...
	if ( $all ) {
		...
		foreach ( $all as $notice ) {
			echo '<div id="" class="' . $notice[1] . ' notice is-dismissible crb-admin-message crb-dismiss-scope"> 
		' . $notice[0] . '<button type="button" class="notice-dismiss crb-dismiss-trigger"><span class="screen-reader-text">Dismiss this notice.</span></button></div>';
		}
	}
	...
	$admin_pages = array(
		'cerber-security'  => array(
			...
			'tab_filter' => function ( $tabs_heading ) {
				...
				$lockouts_count = cerber_blocked_num();

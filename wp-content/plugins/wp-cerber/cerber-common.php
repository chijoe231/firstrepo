...
function crb_get_setting_link( string $setting_id, int $user_id = 0, bool $show_title = false ): string {
	...
	if ( $setting = cerber_settings_config( array( 'setting' => $setting_id ) ) ) {
		if ( $title = $setting['title'] ?? '' ) {
			...
			$fragment = '#' . CRB_SETTING_PREFIX . 'global-' . $setting_id;
		}
	}
...
function cerber_is_table( $table ) {
	global $wpdb;
	if ( ! $wpdb->get_row( "SHOW TABLES LIKE '" . cerber_db_ident( $table ) . "'" ) ) {
		return false;
	}
...
/**
 * Validate and normalize a MySQL identifier (table or column name).
 * Supports simple identifiers and dotted notation.
 *
 * Allowed characters:
 * - ASCII letters A-Z a-z
 * - digits 0-9
 * - underscore _
 * - single dot as a qualifier separator
 *
...
function cerber_db_ident( $ident ): string {
	...
	if ( ! preg_match( '/\A[A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*)?\z/', $ident ) ) {
		return '';
	}
...
final class CRB_JS_Detector {
	...
	public static function cerber_detect_js_code( $string = '' ): bool {
		...
		// Remove all whitespace to simplify regex matching of obfuscated code.
		// This mirrors how JS obfuscators typically pack payloads.
		...
		// ---------------------------------------------------------------------
		// Heuristic 1: JS strings encoded via hex escapes: "\x66\x6f\x6f"
		// ---------------------------------------------------------------------
		// We intentionally match ONLY strings that consist entirely of \xNN sequences.
		// Partial or mixed strings are ignored to avoid decoding junk.
		if ( preg_match_all( '/(["\'])(?:\\\\x[0-9a-fA-F]{2})+\1/m', $compact, $matches ) ) {

			foreach ( $matches[0] as $m ) {

				// Strip quotes and "\x" markers, leaving raw hex digits.
				$hex_str = trim( $m, "\\'\"" );
				$hex_str = str_replace( '\\x', '', $hex_str );

				// If the remaining hex length is odd, decoding would be invalid.
				if ( $hex_str === '' || ( strlen( $hex_str ) % 2 ) !== 0 ) {
					continue;
				}

				// Decode byte-wise. This targets classic JS string obfuscation.
				$hexs = str_split( $hex_str, 2 );
				$decoded = implode( '', array_map( static function ( string $hex ): string {
		...
		// ---------------------------------------------------------------------
		// Heuristic 2: Numeric char codes used for fromCharCode-style obfuscation
		// ---------------------------------------------------------------------
		// Supported formats:
		// - Decimal: 65
		// - Hex:     0x41
		...
		static $charcode_arg_pattern = '/((?:0x[0-9a-fA-F]{1,6})|\d{1,6})\s*(?:,|\))/m';

		if ( preg_match_all( $charcode_arg_pattern, $compact, $matches ) ) {

			// Require a minimum number of tokens.
			// Single or very short sequences are almost always accidental.
			if ( empty( $matches[1] ) || count( $matches[1] ) < self::MIN_CHARCODE_TOKENS ) {
				return false;
			}

			// Decode candidate char codes into a UTF-8 string.
			// The decoder is intentionally tolerant and silent on garbage input.
			$decoded = self::cerber_fromcharcode( implode( ',', $matches[1] ) );

			// Empty or trivially short output is not meaningful JS.
			if ( $decoded === '' || strlen( $decoded ) < self::MIN_DECODED_LENGTH ) {
				return false;
			}

			// Count printable ASCII characters.
			// This is a cheap sanity check to filter binary-like noise.
			// We do NOT try to prove correctness, only plausibility.
			$printable = preg_match_all( '/[A-Za-z0-9]/', $decoded );
			if ( $printable !== false && $printable < self::MIN_PRINTABLE_CHARS ) {
				return false;
			}

			// Final stage: run decoded payload through JS malware patterns.
			// This is intentionally deferred until signal quality is acceptable.
	...
	/**
	 * Heuristic decoder for JS String.fromCharCode-style obfuscation.
	 *
	 * The input may be a clean comma-separated list of codes, or a dirty string.
	 * Only decimal codes and hex codes with 0x prefix are supported.
	 * Octal is intentionally not supported to reduce noise and ambiguity.
	 *
	 * @param string $str Input string with char codes.
	 *
	 * @return string UTF-8 decoded string (best effort).
	 */
	private static function cerber_fromcharcode( string $str ): string {
		...
		/*
		 * Extract tokens in a tolerant way.
		 * This is robust, but still constrained:
		 * - Hex must be 0x... with hex digits
		 * - Decimal must be digits only
		 *
		 * We do not extract negative numbers or scientific notation intentionally.
		 */
		if ( ! preg_match_all( '/0x[0-9a-fA-F]+|\d+/', $str, $m ) ) {
			return '';
		}
		...
		$use_mb = function_exists( 'mb_chr' );
		$out = '';

		foreach ( $tokens as $token ) {

			// Fast prefix check. Token format is guaranteed by the regex above.
			if ( $token[0] === '0' && isset( $token[1] ) && ( $token[1] === 'x' || $token[1] === 'X' ) ) {
				$code = hexdec( $token );
			}
			else {
				// Digits-only token, safe cast.
				$code = (int) $token;
			}

			// Allow only Unicode scalar values to avoid mb_chr() ValueError and invalid output.
			...
			if ( $use_mb ) {
				...
				// Byte-level fallback. Good enough for ASCII-heavy malware keywords.
				$out .= chr( $code & 0xFF );
			}
		}
